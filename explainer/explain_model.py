...
def load_dataset_for_explanation(cache_dir: str,
    ...
    if not cache_files:
        raise FileNotFoundError(
            f"No cached dataset files found in {cache_dir}")
    ...
    if dataset_split == 'all':
        ...
        raise ValueError(
            f"Split '{dataset_split}' not available. Available: {available}")
        ...
        if split in cache_files:
            ...
            try:
                ...
                if hasattr(dataset, 'data_list'):
                    ...
                    raise ValueError(
                        f"Unexpected dataset format: {type(dataset)}")

                print(
                    f"  ✅ Loaded {len(split_data)} graphs from {split} split")
    ...
    if subset_size is not None and len(all_data) > subset_size:
        print(
            f"🎯 Using subset of {subset_size} graphs from {len(all_data)} total")
...
def explain_single_graph(model_path: str,
    ...
    if graph_index >= len(dataset):
        raise ValueError(
            f"Graph index {graph_index} out of range (dataset size: {len(dataset)})")
    ...
    print(
        f"   Nodes: {target_data.x.size(0)}, Edges: {target_data.edge_index.size(1)}")
...
def explain_dataset_batch(model_path: str,
    ...
    # Load model and data
    model, model_config = load_trained_model(model_path, device)
    dataset = load_dataset_for_explanation(
        cache_dir, dataset_split, subset_size=max_graphs)
    ...
    try:
        ...
        for i, (data, graph_id) in enumerate(zip(dataset, graph_ids)):
            ...
            if explanation is None:
                try:
                    ...
                    # Cache the result
                    if cache is not None:
                        cache.save_explanation(
                            explanation, model, explainer.config)
        ...
        successful_explanations = [
            exp for exp in explanations if exp is not None]
        ...
        print(
            f"   Generated {len(successful_explanations)}/{len(dataset)} explanations")
...
"""
    ...
    print(
        f"Average Fidelity+: {stats['quality_metrics']['fidelity_plus']['mean']:.3f}")
    print(
        f"Average Sparsity: {stats['quality_metrics']['sparsity']['mean']:.3f}")
