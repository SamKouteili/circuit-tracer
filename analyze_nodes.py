...
def extract_nodes_with_influence(graph: Dict) -> Dict[str, float]:
    ...
    if 'nodes' in graph:
        ...
        print(
            f"Warning: No 'nodes' key found in graph. Available keys: {list(graph.keys())}")
        return {}
    ...
    if edges_key in graph:
        ...
        for edge in edges:
            ...
            weight = edge.get('weight', edge.get(
                'value', edge.get('strength', 0.0)))
        ...
        print(
            f"Warning: No 'edges' or 'links' key found. Available keys: {list(graph.keys())}")
...
def main():
    ...
    try:
        ...
        print(
            f"Baseline graph loaded: {len(baseline_graph.get('nodes', []))} nodes")
        print(
            f"Injection 1 graph loaded: {len(injection1_graph.get('nodes', []))} nodes")
        print(
            f"Injection 2 graph loaded: {len(injection2_graph.get('nodes', []))} nodes")
        ...
        common_injection_nodes = injection1_node_ids.intersection(
            injection2_node_ids)
        ...
        print(
            f"Nodes unique to injection graphs (not in baseline): {len(unique_to_injections)}")

        if not unique_to_injections:
            print(
                "No nodes found that are in both injection graphs but missing from baseline.")
